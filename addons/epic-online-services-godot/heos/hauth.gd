...
## Same-device playtest windows beyond the first need a distinct anonymous
## Product User ID. The app sets this automatically; normal users keep their
## stable Device ID and never see an account or networking prompt.
var rotate_device_id_before_anonymous_login := false
...
func logout_async() -> EOS.Result:
	...
	var ret: EOS.Result = EOS.Result.InvalidAuth
...
func login_game_services_async(opts: EOS.Connect.LoginOptions) -> bool:
	...
	if login_res == EOS.Result.InvalidUser:
		...
		var create_success: bool = await _create_user_async(login_ret.continuance_token)
...
func login_anonymous_async(p_user_display_name: String, force_new_device_id: bool = false) -> bool:
	var user_display_name: String = p_user_display_name.strip_edges()
	if not p_user_display_name:
		_log.error("User display name is empty")
		return false
	_log.debug("Logging in anonymously...")

	# Device ID storage is machine-wide. Serialize this block so multiple game
	# windows cannot delete/create the credential at the same time.
	var identity_guard := await _acquire_anonymous_identity_guard()
	# A final recovery attempt may explicitly rotate a broken/stale credential.
	# Normal devices retain their stable anonymous profile, while additional
	# same-PC playtest windows still receive distinct identities automatically.
	if rotate_device_id_before_anonymous_login or force_new_device_id:
		...
		if not EOS.is_success(delete_ret):
			_log.debug("Failed to rotate device id: result_code=%s" % EOS.result_str(delete_ret))
	...
	if not EOS.is_success(create_ret) and create_ret.result_code not in [EOS.Result.DuplicateNotAllowed, EOS.Result.AlreadyConfigured]:
	...
	var logged_in := await login_game_services_async(login_opts)
	identity_guard.stop()
	return logged_in


func _acquire_anonymous_identity_guard() -> TCPServer:
	var guard := TCPServer.new()
	while guard.listen(43099, "127.0.0.1") != OK:
...
func get_product_user_info_async(p_product_user_id: String = product_user_id):
